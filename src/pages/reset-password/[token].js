...
const ResetPasswordV2 = () => {
  ...
  const fetchUserDetails = async () => {
    try {
      const user = await API.get(`/auth/admin/verify/${token}`)
      ...
      if (err.response.data.statusCode === 408) {
        ...
        // toast.error('Login Session Expired, Please contact chairman', {
        //   position: 'top-right',
        //   duration: 1000 * 60
        // })
      }
  ...
  const resetPassword = async () => {
    try {
      const user = await API.post(`/auth/admin/reset-password`, { token, password: values.newPassword })
      if (user.status === 201) {
  ...
  return (
    <Box className='content-right' sx={{ backgroundColor: 'background.paper' }}>
      ...
      <Snackbar open={open} onClose={e => setOpen(false)} autoHideDuration={10000}>
        <Alert
          variant='filled'
          severity='error'
        ...
        >
          Login Session Expired, Please contact chairman
        </Alert>
      </Snackbar>
    </Box>
  )
...
ResetPasswordV2.authGuard = false
