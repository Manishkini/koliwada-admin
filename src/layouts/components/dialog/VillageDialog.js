...
const VillageDialog = props => {
  ...
  const createVillage = async obj => {
    try {
      const invitation = await API.post(`/village`, obj)
  ...
  const fetchTehsils = async id => {
    const district = await API.get(`/tehsil/district/${id}`)
    setTehsils(district.data)
  }
  ...
  const initializeStates = async () => {
    ...
    setValue('state', selectedRow?.tehsil?.district?.state?.id)
    await fetchDistricts(selectedRow?.tehsil?.district?.state?.id)
    setValue('district', selectedRow?.tehsil?.district?.id)
    await fetchTehsils(selectedRow?.tehsil?.district?.id)
    setValue('tehsil', selectedRow?.tehsil?.id)
  }
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      fullWidth
                      value={value}
                      label='Village Name Marathi'
                      onChange={onChange}
                      placeholder='Please enter village name in marathi'
